...
use crate::storage_types::{ContentType, DataKey, Trait};
use soroban_sdk::{contract, contractimpl, Address, Bytes, Env, Symbol, TryFromVal, Vec};

#[contract]
pub struct NonFungibleToken;

fn read_nonce(env: &Env, id: &Address) -> i128 {
    todo!();
    // let key = DataKey::Nonce(id.clone());
    // env.storage().get(key).unwrap_or(Ok(0)).unwrap()
}

fn verify_and_consume_nonce(env: &Env, auth: &Address, expected_nonce: i128) {
    todo!();
    // match auth {
...
impl NonFungibleTokenTrait for NonFungibleToken {
    ...
    fn nonce(env: Env, id: Address) -> i128 {
        read_nonce(&env, &id)
    }
    ...
    fn set_admin(env: Env, admin: Address, nonce: i128, new_admin: Address) {
        todo!()
    ...
    fn appr(env: Env, owner: Address, nonce: i128, operator: Address, id: i128) {
        todo!();
    ...
    fn appr_all(env: Env, owner: Address, nonce: i128, operator: Address, approved: bool) {
        todo!();
        //     verify_and_consume_nonce(&env, &owner, nonce);
    ...
    fn transfer(env: Env, from: Address, nonce: i128, to: Address, id: i128) {
        todo!();
    ...
    fn mint(env: Env, admin: Address, nonce: i128, to: Address, id: i128, uri: Bytes) {
        todo!();
        // check_admin(&env, &admin);
        // verify_and_consume_nonce(&env, &admin, nonce);
    ...
    fn mint_next(env: Env, uri: Bytes) {
        todo!();
        // let to = Address::from(env.invoker());
    ...
    fn burn(env: Env, admin: Address, nonce: i128, id: i128) {
        todo!();
        // check_admin(&env, &admin);
        // verify_and_consume_nonce(&env, &admin, nonce);
